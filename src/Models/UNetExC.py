    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, filters=[16, 32, 64], layers=3,
        ...
        for add_fc in add_fc_blocks:
            ...
            for _ in range(self.out_channels):
                if add_fc:
                    fc_layer = []
                    fc_layer.append(nn.Linear(self.fc_in_channels, self.fc_filters[0]))
                    for idx in range(len(self.fc_filters) - 1):
                        fc_layer.append(nn.Linear(self.fc_filters[idx], self.fc_filters[idx + 1]))
                    fc_layer.append(nn.Linear(self.fc_filters[-1], self.fc_out_channels))
